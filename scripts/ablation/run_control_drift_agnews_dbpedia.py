...
parser.add_argument('--training_mode', type=str, default='Homo', choices=['Centralized', 'Homo'], help='Command-line argument.')
parser.add_argument('--data_dir', type=str, default='./data', help='Root directory for datasets.')
parser.add_argument('--lora_r_client', type=int, default=4, help='Client LoRA rank.')
parser.add_argument('--lora_r_server', type=int, default=6, help='Server LoRA rank.')
...
parser.add_argument('--fraction_fit', type=float, default=1.0, help='Fraction of clients used for fitting.')
parser.add_argument('--min_fit_clients', type=int, default=3, help='Minimum number of fit clients.')
parser.add_argument('--min_evaluate_clients', type=int, default=3, help='Minimum number of evaluation clients.')
...
parser.add_argument('--algorithm', type=str, default='ILORA', choices=['FFA-LORA', 'FEDIT', 'FLORA', 'ILORA', 'LoRA_FAIR'], help='Federated optimization algorithm.')
parser.add_argument('--save_dir', type=str, default='/path/to/save_dir', help='Directory for logs and checkpoints.')
parser.add_argument('--dataset', type=str, default='YELP_REVIEW_FULL', choices=['QQP', 'MNLI', 'STS-B', 'WNLI', 'RTE', 'MRPC', 'qnli', 'cola', 'sst2', 'SNLI', 'IMDB', 'AG_NEWS', 'DBPedia', 'ANLI', 'PAWS', 'AMAZON_POL', 'DBPEDIA14', 'YELP_POLARITY', 'YAHOO_ANS', 'TREC', 'SICK', 'YELP_REVIEW_FULL', '20NEWSGROUPS', 'EMOTION', 'sst5', 'MR', 'HateXplain'], help='Dataset name.')
parser.add_argument('--optimizer', type=str, default='SGD', choices=['SGD', 'AdamW'], help='Command-line argument.')
parser.add_argument('--distribution', type=str, default='NON-IID', choices=['IID', 'NON-IID'], help='Command-line argument.')
parser.add_argument('--alpha', type=float, default=0.5, help='Dirichlet alpha for non-IID partitioning.')
parser.add_argument('--heterogeneous_rank', type=str, default='True', choices=['True', 'False'], help='Whether to use heterogeneous LoRA ranks.')
parser.add_argument('--heterogeneous_rank_clients', type=str, default='2,4,6', help='Per-client LoRA ranks, comma-separated.')
parser.add_argument('--use_control', type=str, default='False', choices=['True', 'False'], help='Command-line argument.')
parser.add_argument('--lambda_reg', type=float, default=0.01, help='Command-line argument.')
parser.add_argument('--max_length', type=int, default=64, help='Command-line argument.')
parser.add_argument('--lora_scale_factor', type=float, default=0.5, help='Scale factor for LoRA QR initialization.')
...
if args.heterogeneous_rank == 'True':
    ...
    print('Using a uniform client rank for all clients.')
...
if args.dataset == '20NEWSGROUPS':
    ...
    if os.path.exists(train_dir) and os.path.exists(test_dir):
        ...
        print('Loading 20 Newsgroups via scikit-learn...')
        try:
...
if args.dataset == 'YAHOO_ANS':
    ...
    if not all((os.path.isfile(os.path.join(local_dir, f)) for f in required)):
        raise FileNotFoundError('Missing Yahoo! Answers parquet files in:\n' + local_dir + '\nRequired files: ' + ', '.join(required))
    ...
    print(f"YAHOO_ANS train={len(dataset['train'])}, test={len(dataset['test'])}")
    labels = [int(ex['topic']) for ex in dataset['train']]
    min_label = min(labels)
    max_label = max(labels)
    args.num_labels = max_label - min_label + 1
    print(f'Label range: {min_label} ~ {max_label}, num_labels = {args.num_labels}')
...
if args.dataset == 'SICK':
    ...
    print(f'[SICK] task={sick_task.upper()}, train={len(full_train_dataset)}, test={len(full_test_dataset)}')
if args.dataset == 'YELP_REVIEW_FULL':
    ...
    dataset['train'] = dataset['train'].shuffle(seed=42).select(range(min(SUBSET_SIZE_TRAIN, len(dataset['train']))))
    dataset['test'] = dataset['test'].shuffle(seed=42).select(range(min(SUBSET_SIZE_TEST, len(dataset['test']))))
    print(f"YELP_REVIEW_FULL train={len(dataset['train'])}, test={len(dataset['test'])}")
    args.num_labels = 5
...
if args.dataset == 'ANLI':
    ...
    if {'train_r1', 'dev_r1'} <= available:
        ...
        try:
            ...
            raise ValueError("ANLI test split is missing labels. Use validation or a dev_r* split instead.")
    else:
        raise ValueError(f'Unsupported ANLI split layout: {sorted(list(available))}')
                ...
                if args.use_control == 'True':
                    ...
                    for (client_idx, (_, fit_res)) in enumerate(results):
                        ...
                        try:
                            ...
                            print(f'Client {client_idx} delta_ci error: {str(e)}')
                            continue
...
class CIFAR10Client(NumPyClient):
    ...
    def __init__(self, cid, train_indices, test_indices):
        ...
        print(f'Client {cid} initialized, rank={client_rank}')
    ...
    def fit(self, parameters, config):
        ...
        if args.algorithm == 'ILORA':
            ...
            print('Initializing client model...')
            client_rank = args.heterogeneous_rank_clients[int(self.cid)]
            print('fit client rank:', client_rank)
            self.model = get_model(client_rank)
        ...
        if args.use_control == 'True':
            ...
            try:
                ...
                c = self.ci
                print(f'Client {self.cid}: failed to load global control, using local ci.')
            if len(c) != len(self.ci):
                print('Global control shape mismatch with local ci, falling back to local ci.')
                c = self.ci
        import copy
        if args.use_control == 'True':
