...
parser = argparse.ArgumentParser(description="LoRA experiment configuration")
parser.add_argument("--cuda_device", type=str, default="2", help="Visible GPU device ID.")
parser.add_argument("--training_mode", type=str, default="Homo", choices=["Centralized", "Homo"], help="Training mode.")
parser.add_argument("--data_dir", type=str, default="./data", help="Dataset storage path.")
parser.add_argument("--lora_r_client", type=int, default=4, help="LoRA rank for each local client.")
parser.add_argument("--lora_r_server", type=int, default=4, help="LoRA rank for the server model.")
parser.add_argument("--lora_alpha", type=int, default=16, help="LoRA scaling factor.")
parser.add_argument("--lora_dropout", type=float, default=0.1, help="LoRA dropout rate.")
parser.add_argument("--target_modules", nargs="+", default=["query", "value"], help="Target modules for LoRA adaptation.")
...
parser.add_argument("--fraction_fit", type=float, default=1.0, help="Fraction of clients participating in each round.")
parser.add_argument("--min_fit_clients", type=int, default=3, help="Minimum number of clients used for training.")
parser.add_argument("--min_evaluate_clients", type=int, default=3, help="Minimum number of clients used for evaluation.")
parser.add_argument("--num_rounds", type=int, default=2, help="Total number of communication rounds.")
parser.add_argument("--lr", type=float, default=0.01, help="Learning rate.")
parser.add_argument("--momentum", type=float, default=0.9, help="Momentum value.")
parser.add_argument("--weight_decay", type=float, default=0.0, help="Weight decay.")
parser.add_argument("--local_epochs", type=int, default=1, help="Number of local epochs on each client.")
parser.add_argument("--batch_size", type=int, default=128, help="Batch size.")
parser.add_argument("--num_clients", type=int, default=3, help="Total number of clients.")
parser.add_argument("--algorithm", type=str, default="FEDIT", choices=["FEDIT", "FLORA", "ILORA", "LoRA_FAIR", "FFA-LORA"], help="Federated optimization algorithm.")
...
parser.add_argument("--distribution", type=str, default="NON-IID", choices=["IID", "NON-IID"], help="Data distribution type.")
parser.add_argument("--alpha", type=float, default=0.5, help="Dirichlet alpha controlling non-IID severity.")
parser.add_argument("--heterogeneous_rank", type=str, default="False", choices=["True", "False"], help="Enable or disable heterogeneous ranks.")
parser.add_argument("--heterogeneous_rank_clients", type=str, default="2,4,6", help="Comma-separated list of client LoRA ranks.")
parser.add_argument("--use_control", type=str, default="False", choices=["True", "False"], help="Enable control variates to reduce client drift.")
parser.add_argument("--lambda_reg", type=float, default=0.01, help="Regularization coefficient.")
parser.add_argument("--lora_scale_factor", type=float, default=0.5, help="Scale factor used during QR initialization.")
                ...
                if args.use_control == 'True':
                    ...
                    for (client_idx, (_, fit_res)) in enumerate(results):
                        ...
                        try:
                            ...
                            print(f'Failed to load delta_ci for client {client_idx}: {str(e)}')
                            continue
                    ...
                    if args.use_control == 'True':
                        ...
                        print(f'Saved global control variates to {c_save_path}')
                aggregated_params = None
                for (client_idx, (_, fit_res)) in enumerate(results):
                    client_idx = fit_res.metrics['client_id']
                    p_k = client_weights[client_idx]
...
class CIFAR10Client(NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        if args.algorithm == 'ILORA':
            print('ILORA does not require reinitialization.')
        elif args.algorithm == 'LoRA_FAIR':
            print('LoRA_FAIR does not require reinitialization.')
        elif args.algorithm == 'FFA-LORA':
            print('FFA-LORA does not require reinitialization.')
        elif args.algorithm == 'FEDIT':
            print('FEDIT does not require reinitialization.')
        else:
            print('Model reinitialization is required.')
            client_rank = args.heterogeneous_rank_clients[int(self.cid)]
            self.model = get_model(client_rank)
