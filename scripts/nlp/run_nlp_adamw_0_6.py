...
parser.add_argument('--cuda_device', type=str, default='0', help='GPUID（：3）')
...
parser.add_argument('--model_name', type=str, default='roberta-base', help='（："roberta-base"）')
...
parser.add_argument('--num_rounds', type=int, default=5, help='（：10）')
parser.add_argument('--lr', type=float, default=0.01, help='（：0.01）')
parser.add_argument('--momentum', type=float, default=0.9, help='（：0.9）')
parser.add_argument('--weight_decay', type=float, default=0.0, help='（：0.0）')
...
parser.add_argument('--algorithm', type=str, default='ILORA', choices=['FFA-LORA', 'FEDIT', 'FLORA', 'ILORA', 'LoRA_FAIR'], help='（：FEDIT）')
parser.add_argument('--save_dir', type=str, default='/path/to/save_dir', help='（：/path/to/save_dir）')
parser.add_argument('--dataset', type=str, default='YELP_REVIEW_FULL', choices=['QQP', 'MNLI', 'STS-B', 'WNLI', 'RTE', 'MRPC', 'qnli', 'cola', 'sst2', 'SNLI', 'IMDB', 'AG_NEWS', 'DBPedia', 'ANLI', 'PAWS', 'AMAZON_POL', 'DBPEDIA14', 'YELP_POLARITY', 'YAHOO_ANS', 'TREC', 'SICK', 'YELP_REVIEW_FULL', '20NEWSGROUPS', 'EMOTION', 'sst5', 'MR', 'HateXplain'], help='（：DBPedia）')
...
if args.dataset == '20NEWSGROUPS':
    ...
    if os.path.exists(train_dir) and os.path.exists(test_dir):
        ...
        print(f'20 Newsgroups ，: {args.num_labels}')
        print(f': {len(train_data)}')
        print(f': {len(test_data)}')
    else:
        print('20 Newsgroups，scikit-learn...')
        try:
...
if args.dataset == 'QQP':
    ...
    args.num_labels = 2
    print(f'QQP1/4:  {train_subset_size} ,  {test_subset_size} ')
...
if args.dataset == 'SICK':
    ...
    if not os.path.isfile(raw_txt):
        raise FileNotFoundError(f'[SICK] ：{raw_txt}\n SICK.txt  {{sick_dir}} 。')
    ...
    if 'entailment_label' not in df.columns:
        raise KeyError('[SICK] ， entailment_label ')
...
if args.dataset == 'ANLI':
    ...
    if {'train_r1', 'dev_r1'} <= available:
        ...
        try:
            ...
            raise ValueError(f"ANLI  'test' ，。 'validation'  'dev_r*'。")
    else:
...
class CIFAR10Client(NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        if args.use_control == 'True':
            ...
            print(f' {self.cid} Δci {delta_ci_path}')
