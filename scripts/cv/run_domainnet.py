...
parser.add_argument('--cuda_device', type=str, default='2', help='GPUID（：3）')
parser.add_argument('--training_mode', type=str, default='Homo', choices=['Centralized', 'Homo'], help=': Centralized()Homo() (: Homo)')
parser.add_argument('--data_dir', type=str, default='./data', help='（："./data"）')
parser.add_argument('--lora_r_client', type=int, default=4, help='LoRA（：4）')
parser.add_argument('--lora_r_server', type=int, default=6, help='LoRA（：4）')
parser.add_argument('--lora_alpha', type=int, default=16, help='LoRA（：16）')
parser.add_argument('--lora_dropout', type=float, default=0.1, help='LoRADropout（：0.1）')
parser.add_argument('--target_modules', type=list, default=['query', 'value'], help='LoRA（：["query", "value"]）')
parser.add_argument('--model_name', type=str, default='vit-base-patch16-224', help='ViT（："vit-base-patch16-224","swin-base-patch4-window7-224"）')
parser.add_argument('--num_labels', type=int, help='（CIFAR-1010，CIFAR-100100）')
parser.add_argument('--fraction_fit', type=float, default=1.0, help='（：1.0）')
parser.add_argument('--min_fit_clients', type=int, default=3, help='（：3）')
parser.add_argument('--min_evaluate_clients', type=int, default=3, help='（：3）')
parser.add_argument('--num_rounds', type=int, default=2, help='（：10）')
parser.add_argument('--scheduler', type=str, default='CosineAnnealing', choices=['None', 'StepLR', 'CosineAnnealing', 'Exponential', 'ReduceLROnPlateau'], help=' (: None)')
parser.add_argument('--step_size', type=int, default=30, help='StepLR（：30）')
parser.add_argument('--gamma', type=float, default=0.1, help='（：0.1）')
parser.add_argument('--t_max', type=int, default=50, help='CosineAnnealingLRT_max（：50）')
parser.add_argument('--min_lr', type=float, default=0.0001, help='（：0.0001）')
parser.add_argument('--patience', type=int, default=10, help='ReduceLROnPlateau（：10）')
parser.add_argument('--data_subset_ratio', type=float, default=0.8, help=' (0.0-1.0，: 1.0 )')
parser.add_argument('--lr', type=float, default=0.01, help='（：0.01）')
parser.add_argument('--momentum', type=float, default=0.9, help='（：0.9）')
parser.add_argument('--weight_decay', type=float, default=0.01, help='（：0.0）')
parser.add_argument('--local_epochs', type=int, default=1, help='（：1）')
parser.add_argument('--batch_size', type=int, default=128, help='/（：128）')
parser.add_argument('--num_clients', type=int, default=3, help='（：3）')
parser.add_argument('--algorithm', type=str, default='LoRA_FAIR', choices=['FEDIT', 'FLORA', 'ILORA', 'LoRA_FAIR', 'FFA-LORA'], help='（：FEDIT）')
parser.add_argument('--save_dir', type=str, default='/path/to/save_dir', help='（：/path/to/save_dir）')
parser.add_argument('--dataset', type=str, default='domainnet_quickdraw', choices=['cifar10', 'cifar100', 'MNIST', 'STL10', 'SVHN', 'tiny-imagenet-200', 'domainnet_clipart', 'domainnet_infograph', 'domainnet_painting', 'domainnet_quickdraw', 'domainnet_real', 'domainnet_sketch'], help='（：cifar10）')
parser.add_argument('--optimizer', type=str, default='SGD', choices=['SGD', 'AdamW'], help=': SGD  AdamW (: SGD)')
parser.add_argument('--distribution', type=str, default='NON-IID', choices=['IID', 'NON-IID'], help='IIDNON-IID')
parser.add_argument('--alpha', type=float, default=0.5, help='Non-IID，Non-IID')
parser.add_argument('--heterogeneous_rank', type=str, default='True', choices=['True', 'False'], help='True，False')
parser.add_argument('--heterogeneous_rank_clients', type=str, default='2,4,6', help='LoRA（：4）')
parser.add_argument('--use_control', type=str, default='False', choices=['True', 'False'], help='control (: False)')
parser.add_argument('--lambda_reg', type=float, default=0.01, help='LoRA_FAIR')
parser.add_argument('--lora_scale_factor', type=float, default=0.5, help='QRs（：1.0）')
from tensorboardX import SummaryWriter
parser.add_argument('--log_dir', type=str, default='./runs', help='TensorBoard （ ./runs）')
...
print(f':  {len(full_train_dataset)} ,  {len(full_test_dataset)} ')
